# from: https://github.com/acids-ircam/nn_tilde/blob/master/python_tools/module.py
...
class Module(torch.nn.Module):
    ...
    def register_method(
        ...
        if test_method:
            ...
            x = torch.zeros(1, in_channels, test_buffer_size // in_ratio).to(test_device)
            ...
            if cc.MAX_BATCH_SIZE > 1:
                ...
                x = torch.zeros(4, in_channels, test_buffer_size // in_ratio).to(test_device)
                y = getattr(self, method_name)(x)
